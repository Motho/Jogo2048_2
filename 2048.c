...
void printa_tabela(int linhas, int colunas, struct matriz){
...
void move_direita(int tabuleiro[N][N],int largura,int altura) {
...
void move_esquerda(int tabuleiro[altura][largura], int largura, int altura) {
...
void move_cima(int tabuleiro[altura][largura],int altura,int largura) {
    int i, j, k;
    for (j = 0; j < largura; j++) {
...
void move_baixo(int tabuleiro[altura][largura],int altura,int largura) {
    int i, j, k;
    for (j = 0; j < largura; j++) {
...
int surge_peca(int largura, int altura){
    int numero;
    int i, j;
...
//Randomizacao
    ...
    if(numero>= 9 && tabuleiro.posicao[i][j]== 0) tabuleiro.posicao[i][j]= 4; // caso seja 9 ou 10 printa 4 na posicao  (10% de chance)
    if(numero<= 8 && tabuleiro.posicao[i][j]== 0) tabuleiro.posicao[i][j]= 2; // caso seja de 1 até 8 printa 2 na posicao  (90% de chance)
}
 
int estado_do_jogo(int largura, int altura){	//checa condicoes de vitoria e derrota alem de pegar o nome do jogador vencedor
...
int jogo(int alt, int lar,int tabuleiro[alt][lar]){// as mecanicas do jogo comecam aqui
    ...
    while(3){ 
		estado_do_jogo(lar,alt);
		if(estado_do_jogo(lar,alt) == 1){
		printf("Voce venceu. Insira seu nome;");
		return 0;
		}
		if(estado_do_jogo(lar,alt) == 2) return 0;// vai encerrar o programa caso o num de pecas ocupe todo o tabuleiro

    surge_peca(lar,alt);

    //parte responsavel por printar a tabela

	printa_tabela(alt,lar);
