...
void move_cima(int tabuleiro[N][N]) {
    ...
    for (j = 0; j < LAR; j++) {
        for (i = 0; i < ALT - 1; i++) {
            for (k = i + 1; k < LAR; k++) { // 'k' representado por 'i + 1' para ser a posição acima    
...
void move_baixo(int tabuleiro[ALT][LAR]) {
    int i, j, k;
    for (j = 0; j < LAR; j++) {
        for (i = ALT - 1; i > 0; i--) {
...
//Randomizacao
    ...
    i = rand()%ALT; // faz cair em uma posicao aleatoria do tabuleiro
    j = rand()%LAR; // faz cair em uma posicao aleatoria do tabuleiro
...
int main(int argc,char* argv[2]){
  ...
  #define ALT alt
  #define LAR larguraura

  for(int i= 0; i<ALT;i++){ //preenche o vetor tabuleiro com 0
    for(int j= 0;j<LAR;J++){
        tabuleiro.posicao[i][j]= 0;
    }
  }
